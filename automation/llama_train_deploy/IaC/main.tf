module "llama_train_deploy" {
  ...
  prefix                     = "llama_train_deploy"
  container_registry         = var.container_registry
  container_repository       = "llama-train-deploy"
...
output "llama_train_deploy_function_url" {
  value = module.llama_train_deploy.function_url
}
...
terraform {
  backend "s3" {
    ...
    key     = "llama_train_deploy/tf.state"
